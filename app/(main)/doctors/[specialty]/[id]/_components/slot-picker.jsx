  ...
  return (
    <div className="space-y-6">
      <Tabs
        ...
        className="w-full"
      >
        <TabsList className="w-full overflow-x-auto scrollbar-hide flex-nowrap flex p-1 mb-2 h-16 sm:h-12">
          {days.map((day) => (
            <TabsTrigger
              key={day.date}
              value={day.date}
              disabled={day.slots.length === 0}
              className={`min-w-max flex-shrink-0 px-3 py-2 rounded ${
            ...
            >
              ...
              {day.slots.length > 0 && (
                <div className="ml-1 sm:ml-2 bg-emerald-900/30 text-emerald-400 text-xs px-2 py-1 rounded">
                  {day.slots.length}
                </div>
              )}
            </TabsTrigger>
